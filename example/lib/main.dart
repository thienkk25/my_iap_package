...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  // 1. Khởi tạo Manager
  // Bật enableMockMode = true để giả lập mua hàng trên máy ảo (Simulator) không bị lỗi.
  final IapManager _iapManager = IapManager(enableMockMode: true);
  ...
  Future<void> _initIap() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Lỗi khởi tạo IAP: $e')),
      );
      setState(() => _isLoading = false);
    }
  }
        ...
        children: [
          ...
          Container(
            ...
            child: StreamBuilder<UserEntitlement>(
              ...
              builder: (context, snapshot) {
                ...
                return Column(
                  children: [
                    const Icon(Icons.lock_outline, color: Colors.black54, size: 64),
                    const SizedBox(height: 12),
                    Text(
                      'TÀI KHOẢN MIỄN PHÍ',
                      style: Theme.of(context).textTheme.titleLarge?.copyWith(
                    ...
                    const SizedBox(height: 8),
                    const Text('Hãy mua gói cước bên dưới để mở khoá tính năng.'),
                  ],
                );
              },
            ),
          ...
          const Padding(
            ...
            child: Align(
              ...
              child: Text('Danh Sách Gói Cước', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
          ),
          const SizedBox(height: 8),
                        ...
                        child: ListTile(
                          ...
                          trailing: ElevatedButton(
                            ...
                            onPressed: () async {
                              try {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('Đang xử lý mua: ${product.title}...')),
                                );
